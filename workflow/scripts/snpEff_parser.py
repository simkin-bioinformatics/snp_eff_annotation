...
The logic of this code has to follow a relatively complex process when there are
multiple alternate alleles at a single genomic coordinate - if only one of the
alternate alleles is shown in the vcf file, the code needs to assign a nonzero
depth to the second alternate allele (which is not shown in the vcf file). If the
first alternate allele is annotated by snpEff, the first alternate allele will then
be counted as present twice (once in the snpEff annotations and once from the
targets.tsv file). This second copy of the allele then needs to be removed in
favor of the snpEff counts.
...
def make_targets_dict(targets_tsv, gene_mappings):
    ...
    for line_number, line in enumerate(open(targets_tsv)):
        if line_number > 0:
            ...
            targets_dict[chrom][pos][ref][alt] = [
                gene_ID,
                gene_name,
                mutation_name,
                "missense_mutation",
...
def check_unannotated_targeted(
    vcf_line, targets_dict, depth_dict, samples, mutation_number
):
    """
    ...
    if (
        chrom in targets_dict
        and pos in targets_dict[chrom]
        and ref_allele in targets_dict[chrom][pos]
    ):
        for alt_allele in targets_dict[chrom][pos][ref_allele]:
            ...
            for sample_number, sample in enumerate(samples):
                ...
                depth_dict[sample][mutation_number] = [
                    f"{ref + alt},{ref},{alt}"
                ] + header_list
            mutation_number += 1
...
def grab_mutation_names(sample_dict):
    ...
    return [
        sample_dict[first_sample][mutation][3] for mutation in sample_dict[first_sample]
    ]
...
def fill_in_missing_values(
    protein_dict, targets_dict, targeted_depth_dict, targeted_mutations
):
    """
                ...
                for column_number, column in enumerate(unparsed_snpeff):
                    ...
                    for sample_number, sample in enumerate(samples):
                        ...
                        if column_number % 15 == 14:
                            ann_dict[sample][parsed_counter] = [
                                line[8] + ";" + line[9 + sample_number]
                            ] + header
            target_depth_dict, mutation_number = check_unannotated_targeted(
                line, targets_dict, target_depth_dict, samples, mutation_number
            )
...
def parse_annotations(ann_dict, gene_mappings, targeted_mutations):
    ...
    for sample in ann_dict:
        ...
        for mut in ann_dict[sample]:
            ...
            if (
                len(columns) == 16 and columns[11]
            ):  # item 11 is only populated if the mutation is protein-coding
...
merged_dict, sorting_list = fill_in_missing_values(
    protein_dict, targets_dict, targeted_depth_dict, targeted_mutations
)
